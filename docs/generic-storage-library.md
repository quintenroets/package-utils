...
Escape-hatch SQL goes through the same `Database` (one engine, one transaction), so it shares the cache and transaction semantics. The `RecordPlan` applies an idempotent per-column converter because raw `text()` yields DBAPI-native scalars (bool as `0`, datetime as ISO string) that the Core-typed `Store` path already decodes.

`fetch_all`/`fetch_one` return SQLAlchemy **`Row`**, not `RowMapping`: read a column as `row.week` (or `row[0]`), **not** `row["week"]`. `instances_from` handles the name lookup itself, so a hand-written projection may list its columns in any order.
