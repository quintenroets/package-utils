...
## 2026-07-25 — positional rows (`fetch_all` returns `Row`, not `RowMapping`)

Follow-on to the read plan below. `fetch_row_list` built a `RowMapping` per row (`list(result.mappings())`); the mapping view costs more than the rows themselves and every lookup then hashes a column name. `fetch_all`/`fetch_one` now return SQLAlchemy `Row` — `result.all()` — and `RecordPlan.indexed(index_of)` rebinds each `FieldPlan.key` from a column name to its position in the projection. `Store` supplies the projection order it already knows (`child.column_names()` for `select(*columns)`, `table.c.keys()` for `select(table)`), so nothing is inferred from the result. Measured on the same 19,000 rows: fetch 11.6 → 7.1 ms, per-row access 13.9 → 8.2 ms, `read_dict` ~30 → **23.2 ms** (~52 ms before both changes).

`Row` is strictly richer than `RowMapping` — positional *and* attribute access — so the escape hatch keeps named access as `row.week`; `instances_from` reads through `row._mapping` because a hand-written projection's column order belongs to the caller. **`row["name"]` no longer works on a fetched row**; that cost 11 call sites in powerplan's `adherence.py`, mechanically converted to `row.name`. Done deliberately before release — the storage module is unreleased, so the contract was still free to change.

## 2026-07-25 — per-class read plan (mapping layer 3× faster)

`instance_from` recomputed per **row** what is fixed per **class**: `is_enum_name` (an `isinstance` + `issubclass` per field per row), `path.split(ATTR_DELIMITER)` in `assign_path`, `coerce`'s four type tests, and `construct`'s `nested_record_fields` lookup — 475,000 calls each on a 19,000-row × 5-field read. Replaced by a `@typed_cache`d `RecordPlan` per class (`FieldPlan(attr, column, convert)` plus nested sub-plans) that `store.py` resolves **once per scope read** via `element_reader(spec)`, which hoists the spec dispatch and the `field_.name` lookups out of the per-row path too. Measured on 19,000 flat 5-scalar rows: mapping 32.1 → 11.6 ms, `read_dict` ~52 → ~30 ms (the remaining 12 ms floor is Core's `fetch_all`).

Conversion now resolves to a converter **or `None`**, and `None` skips the call entirely — the `str`/`int`/`float` majority costs one `is not None` test per column instead of four type tests. `coerce`/`is_enum_name`/`construct` are gone; `assign_path` stays only because serie imports it. `instance_from` was dropped (unreleased, no consumers) — `instances_from` and `element_reader` are the API.

> **Do not re-propose a `cls(**row)` fast path for flat records.** Measured at 13.5 ms vs the plan walk's 8.0 ms — splatting a full `RowMapping` into kwargs costs more than walking a precomputed plan — and it breaks the NULL→default invariant, since a NULL column must be *absent* from the constructor call, not passed as `None`. Codegen (`exec` of a generated mapper) reaches the 5.9 ms hand-written ceiling but was rejected: ~2 ms on a whole-manifest load does not justify the machinery. The plan's representation is not worth tuning either — frozen dataclass, slots dataclass, `NamedTuple` and a closure all measured within noise (10.7–11.3 ms).
