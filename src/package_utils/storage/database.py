...
if TYPE_CHECKING:
    ...
    from sqlalchemy import Executable, Row, Table
...
class Database:
    ...
    def fetch_one(self, statement: Executable, params: Any = None) -> Row[Any] | None:
        return cast("Row[Any] | None", self.fetch(statement, params, fetch_first_row))

    def fetch_all(self, statement: Executable, params: Any = None) -> list[Row[Any]]:
        return cast("list[Row[Any]]", self.fetch(statement, params, fetch_row_list))
...
def fetch_row_list(result: Any) -> list[Any]:
    """Positional `Row` tuples, not `RowMapping`s.

    Building the mapping view costs more than the rows themselves, and the
    mapping layer reads by index anyway; `Row` still exposes its columns as
    attributes (`row.name`) and as a mapping (`row._mapping`) when a caller
    wants them by name.
    """
