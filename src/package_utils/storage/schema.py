...
class RecordSpec(ChildSpec):
    ...
    def column_names(self) -> list[str]:
        return [field_.name for field_ in attr_field_map(self.cls).values()]

    def value_columns(self, element: Any) -> dict[str, Any]:
        return {
            field_.name: resolve_attr(element, attr)
            for attr, field_ in attr_field_map(self.cls).items()
...
class Schema:
    ...
    def scope_of_dataclass(self, cls: type) -> Scope:
        fields = attr_field_map(cls)
        if fields:
            scope = self.scope_of_field(next(iter(fields.values())))
        else:
...
@typed_cache
def attr_field_map(cls: type, prefix: str = "") -> dict[str, Field]:
    """Map each scalar attribute path to its column, descending into nested records.

    The `Field` carries the column's name and type, both filled in by
    `assign_names` when the owning namespace is registered as a scope.
    """
    hints = get_type_hints(cls, include_extras=True)
    ...
    for attr_name, hint in hints.items():
        ...
        if is_dataclass(hint):
            ...
            result.update(attr_field_map(nested, f"{path}{ATTR_DELIMITER}"))
