...
from .mapping import element_reader
from .schema import (
    ChildSpec,
    Field,
    Key,
...
@cache
def load_upsert_insert(dialect_name: str) -> Any:
    # a dialect module is expensive to import and only the one in use is needed
    return import_module(UPSERT_DIALECT_MODULES[dialect_name]).insert
...
class Store:
    ...
    def read(self, spec: Any) -> Any:
        ...
        return self._element_of(child, row, names)

    def _element_of(self, spec: ChildSpec, row: Any, order: Sequence[str]) -> Any:
        if row is None:
            element = None
        else:
            element = element_reader(spec, order)(row)
    ...
    def _write_instance(self, instance: Any) -> None:
        ...
        instance_keys = [
            ...
            for attr, field_ in attr_field_map(type(instance)).items()
    ...
    def _read_scope(self, spec: ChildSpec) -> dict[tuple[Any, ...], Any]:
        ...
        order = list(table.c.keys())  # `select(table)` projects the table's own order
        read_element = element_reader(spec, order)
        key_indices = [order.index(field_.name) for field_ in remaining]
        elements = {
            tuple(row[index] for index in key_indices): read_element(row)
            for row in rows
        }
    ...
    def _upsert(self, scope: Scope, values: dict[str, Any]) -> None:
        ...
        insert_ = load_upsert_insert(self.database.dialect_name)
