...
from package_utils.storage.schema import attr_field_map, child_specs
...
def test_instances_from_coerces_raw_datetime(db: Database) -> None:
    at = datetime(2026, 6, 13, 9, 30, tzinfo=UTC)
...
def test_null_column_falls_back_to_dataclass_default(db: Database) -> None:
    db.run(text("INSERT INTO node (key) VALUES ('sparse')"))
    sparse = Store(db)["sparse"].read(Node)
    assert sparse is not None
    assert sparse.label == ""
    assert sparse.active is False


def test_all_null_nested_record_falls_back_to_default(db: Database) -> None:
    db.run(text("INSERT INTO node (key) VALUES ('sparse')"))
    sparse = Store(db)["sparse"].read(Node)
    assert sparse is not None
    assert sparse.meta == Meta()


def test_partly_null_nested_record_keeps_remaining_defaults(db: Database) -> None:
    db.run(text("INSERT INTO node (key, meta_tag) VALUES ('half', 'only-tag')"))
    half = Store(db)["half"].read(Node)
    assert half is not None
    assert half.meta == Meta(tag="only-tag", count=0)


def test_assign_path_creates_missing_parents() -> None:
    data: dict[str, Any] = {"meta": {"tag": "kept"}}
    assign_path(data, "meta.count", 3)
    assign_path(data, "nested.deep.leaf", "value")
    assert data == {
        "meta": {"tag": "kept", "count": 3},
...
def test_datetime_key_round_trip(db: Database) -> None:
    at = datetime(2026, 6, 13, 9, 30, 15, tzinfo=UTC)
...
def test_satellite_round_trip_and_cascade(db: Database) -> None:
    at = datetime(2026, 6, 18, 9, 30, tzinfo=UTC)
...
def test_attr_field_map_skips_non_annotated() -> None:
    @dataclass
    class Mixed:
        x: int = 0
        y: Annotated[int, Field(int)] = 0

    result = attr_field_map(Mixed)
    assert "x" not in result
    assert "y" in result


def test_attr_field_map_skips_annotated_without_field() -> None:
    @dataclass
    class Tagged:
        x: Annotated[int, "not_a_field"] = 0
        y: Fields.node.label = ""

    result = attr_field_map(Tagged)
    assert "x" not in result
    assert "y" in result
